...
The plugin is configurable on a project level.  Configurable items include: enable/disable the plugin, default assignee if SCM author doesn't exist in SonarQube, and 'override' assignee that is useful for testing so that other users are not spammed with notifications.
